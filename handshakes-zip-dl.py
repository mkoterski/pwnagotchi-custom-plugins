...
{% block content %}
    <a class="ui-btn" href="/plugins/handshakes-zip-dl/all">Download Zip</a>
    <ul id="list" data-role="listview" style="list-style-type:disc;">
        {% for handshake in handshakes %}
            <li class="file">
                <a href="/plugins/handshakes-zip-dl/{{handshake}}">{{handshake}}</a>
            </li>
        {% endfor %}
    </ul>
{% endblock %}
...
class HandshakesDL(plugins.Plugin):
    ...
    def on_loaded(self):
        logging.info("[HandshakesZipDL] plugin loaded")
